...
import { FiCheckCircle, FiDollarSign, FiCalendar, FiClock } from 'react-icons/fi';
...
export function InstallmentDetailDrawer({
  ...
  const remainingBalNum = Number(selectedPlan.remainingBalance);
  const isComplete = selectedPlan.status === 'COMPLETE' || remainingBalNum <= 0;

  // Find closed / completed date
  const lastPaidScheduleItem = [...schedule].reverse().find((s) => s.paid && s.paidAt);
  const closedDateStr = lastPaidScheduleItem?.paidAt
    ? new Date(lastPaidScheduleItem.paidAt).toLocaleDateString()
    : new Date(selectedPlan.updatedAt || selectedPlan.createdAt).toLocaleDateString();

  // Total paid against the sale
  const salePayments = selectedPlan.sale?.payments || [];
  const totalPaidAgainstSale = salePayments.reduce((acc: number, p: any) => acc + Number(p.amount), 0);

  // Next monthly installment balance due
  const nextUnpaidInst = schedule.find((s) => !s.paid);
  const nextMonthlyBalance = nextUnpaidInst
    ? Math.max(0, (nextUnpaidInst.amount + (nextUnpaidInst.lateFee || 0)) - (nextUnpaidInst.paidAmount || 0))
    : 0;
    ...
    <div className="flex flex-col min-h-0 bg-surface p-5 overflow-y-auto">
      <div className="space-y-4">
        ...
        <div className="border-b border-border pb-3">
          ...
          {/* If Plan is COMPLETE, prominent completion & closed banner */}
          {isComplete ? (
            <div className="mt-3 flex items-start gap-2.5 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs animate-in fade-in duration-150">
              <FiCheckCircle className="h-5 w-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-bold text-sm">Agreement Fully Settled &amp; Closed</p>
                <p className="text-[11px] text-muted mt-0.5">
                  Completed &amp; closed on <strong className="text-ink">{closedDateStr}</strong>.
                  The customer has paid the full remaining balance. No further payments required.
                </p>
              </div>
            </div>
          ) : null}
        ...
        {/* Financial Breakdown: Total Payable, Total Paid, Remaining Balance */}
        <div className="rounded-xl bg-canvas border border-border p-3 space-y-1 text-xs">
          ...
          <div className="flex justify-between text-emerald-600 dark:text-emerald-400 font-medium">
            <span>Total Paid Towards Agreement:</span>
            <span className="font-mono">
              Rs {(Number(selectedPlan.totalPayable || selectedPlan.remainingBalance) - remainingBalNum).toFixed(2)}
            </span>
          </div>
          {nextMonthlyBalance > 0 && !isComplete ? (
            <div className="flex justify-between text-muted">
              <span>Next Monthly Installment:</span>
              <span className="font-mono font-bold text-ink">
                Rs {nextMonthlyBalance.toFixed(2)}
              </span>
            </div>
          ) : null}
          <div className="border-t border-border pt-1.5 flex justify-between items-baseline font-bold text-ink">
            ...
            <span
              className={`text-base font-mono font-extrabold ${
                remainingBalNum === 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-primary'
              }`}
            >
              {remainingBalNum === 0 ? 'Rs 0.00 (Fully Paid)' : `Rs ${remainingBalNum.toFixed(2)}`}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            {isComplete && (
              <span className="text-[10px] text-emerald-600 font-semibold">Closed on {closedDateStr}</span>
            )}
          </div>
            ...
            <table className="w-full border-collapse text-left text-[11px] text-ink bg-canvas">
              <thead>
                <tr className="border-b border-border bg-surface font-bold text-muted uppercase text-[9px]">
                  ...
                  <th className="px-2.5 py-2">Paid / Settled</th>
              ...
              <tbody className="divide-y divide-border">
                {schedule.map((inst, idx) => (
                  <tr key={idx} className={inst.paid ? 'bg-emerald-500/5' : ''}>
                    <td className="px-2.5 py-2 font-bold">{inst.installmentNumber}</td>
                    <td className="px-2.5 py-2 text-muted">
                      {new Date(inst.dueDate).toLocaleDateString()}
                    </td>
                    <td className="px-2.5 py-2 font-mono text-[10px] text-ink">
                    ...
                    <td className="px-2.5 py-2 font-mono">Rs {inst.amount.toFixed(2)}</td>
                    <td className="px-2.5 py-2">
                      {inst.earlySettlement ? (
        ...
        {/* Sale Payments History (Permanent Reflection in Sale Record) */}
          ...
          <div className="space-y-1.5 border-t border-border pt-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-ink uppercase tracking-wider flex items-center gap-1">
                ...
                <span>Sale Payments Record ({salePayments.length})</span>
              </h3>
              <span className="text-[10px] font-mono text-muted">
                Total Paid: Rs {totalPaidAgainstSale.toFixed(2)}
              </span>
            </div>

            <div className="rounded-xl border border-border overflow-hidden max-h-36 overflow-y-auto">
              <table className="w-full border-collapse text-left text-[11px] text-ink bg-canvas">
                <thead>
                  <tr className="border-b border-border bg-surface font-bold text-muted uppercase text-[9px]">
                ...
                <tbody className="divide-y divide-border">
                  {salePayments.map((pay: any) => (
                    <tr key={pay.id}>
                      <td className="px-2.5 py-1.5 text-muted font-mono text-[10px]">
                        {new Date(pay.createdAt).toLocaleString()}
                      </td>
                      <td className="px-2.5 py-1.5 font-semibold text-[10px]">
                        {pay.method}
                      </td>
                      <td className="px-2.5 py-1.5 text-right font-mono font-bold text-ink">
        ...
        {!isComplete ? (
          <form onSubmit={onRecordPayment} className="space-y-2.5 border-t border-border pt-3">
            <div className="flex items-center justify-between flex-wrap gap-1.5">
              <h3 className="text-xs font-bold text-ink uppercase tracking-wider">
                Record Payment (Cash / Bank Only)
              </h3>
              <div className="flex items-center gap-1.5 flex-wrap">
                {nextMonthlyBalance > 0 && (
                  <button
                    type="button"
                    onClick={() => onPayAmountChange(nextMonthlyBalance.toFixed(2))}
                  ...
                  >
                    Monthly: Rs {nextMonthlyBalance.toFixed(2)}
                  </button>
                )}
                <button
                  type="button"
                  onClick={() => onPayAmountChange(remainingBalNum.toFixed(2))}
                  className="px-2 py-0.5 text-xs font-mono font-medium rounded-lg border border-border bg-canvas hover:bg-surface hover:border-ink transition-colors cursor-pointer"
                >
                  Full: Rs {remainingBalNum.toFixed(2)}
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <Input
                  ...
                  max={remainingBalNum}
                  step="any"
                  placeholder={`Amount (max Rs ${remainingBalNum.toFixed(2)})`}
                  value={payAmount}
