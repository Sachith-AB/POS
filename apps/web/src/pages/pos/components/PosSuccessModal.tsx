import { FiUser, FiCheckCircle, FiRepeat } from 'react-icons/fi';
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-2xl">
        <div className="text-center mb-5">
          ...
          <div className="flex flex-col items-center gap-1.5 mt-3">
            ...
            {lastCompleted.tradeInDevice ? (
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 text-blue-700 dark:text-blue-300 text-xs font-semibold border border-blue-500/20">
                <FiRepeat className="h-3.5 w-3.5" />
                <span>
                  Trade-In: {lastCompleted.tradeInDevice.deviceInfo}
                  {lastCompleted.tradeInDevice.imei ? ` (${lastCompleted.tradeInDevice.imei})` : ''}
                </span>
                <span className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">
                  (-Rs {lastCompleted.tradeInDevice.tradeInValue.toFixed(2)})
                </span>
              </div>
            ) : null}
          </div>
